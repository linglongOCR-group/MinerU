...
def main(ctx,
    ...
    with gr.Blocks(**_blocks_kwargs) as demo:
        ...
        with gr.Column(elem_classes=["mineru-advanced-popover"]):
            with gr.Column(elem_classes=["mineru-advanced-card"]):
                with gr.Row(visible=False) as client_options:
                    ...
                    layout_url = gr.Textbox(
                        label=i18n("layout_server_url"),
                        value='',
                        placeholder='http://localhost:30000',
                        info=i18n("layout_server_url_info"),
                    )
                    recognition_url = gr.Textbox(
                        label=i18n("recognition_server_url"),
                        value='',
                        placeholder='http://localhost:30000',
                        info=i18n("recognition_server_url_info"),
                    )
                with gr.Group():
        ...
        change_bu.click(
            ...
            inputs=[input_file, max_pages, is_ocr, formula_enable, table_enable, image_analysis, language, backend, url, layout_url, recognition_url],
